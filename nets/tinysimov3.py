...
class Conv(torch.nn.Module):
    def __init__(self, in_ch, out_ch, k=1, s=1, p=None, d=1, g=1):
        ...
        self.fused_conv = None  # Stores fused weights for inference

    def forward(self, x):
        if self.training:
            # Training path: Conv -> BN -> SiLU
            return self.relu(self.norm(self.conv(x)))
        else:
            # Inference path: Fused Conv -> SiLU
            if self.fused_conv is None:
                # Fuse conv and bn if not already fused
                self.fuse()
            return self.relu(self.fused_conv(x))

    def fuse(self):
        # Fuse Conv and BN into one Conv layer
        self.fused_conv = fuse_conv(self.conv, self.norm)
        return self
...
class YOLO(torch.nn.Module):
    ...
    def fuse(self):
        # Fuse all Conv layers for inference
        for m in self.modules():
    ...
    def train(self, mode=True):
        # Clear fused weights when switching to training mode
        if mode:
            for m in self.modules():
                if isinstance(m, Conv) and m.fused_conv is not None:
                    m.fused_conv = None
