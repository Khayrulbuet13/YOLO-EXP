...
class ComputeLoss:
    ...
    def assign(self, pred_scores, pred_bboxes, true_labels, true_bboxes, true_mask, anchors):
        ...
        if self.num_max_boxes == 0:
            ...
            #     torch.zeros_like(pred_scores[..., 0], dtype=torch.bool).to(device)
            # )
            
            # Return empty tensors with proper gradient tracking
            return (
...
def make_anchors(x, strides, offset=0.5):
    """Generate anchors for single feature map"""
    anchor_points, stride_tensor = [], []
    
    # x is list with single feature map tensor
    if isinstance(x, list) or isinstance(x, tuple):
        x = x[0]
    
    _, _, h, w = x.shape
    sx = torch.arange(end=w, dtype=x.dtype, device=x.device) + offset
    sy = torch.arange(end=h, dtype=x.dtype, device=x.device) + offset
    sy, sx = torch.meshgrid(sy, sx)
    
    anchor_points = torch.stack((sx, sy), -1).view(-1, 2)
    stride_tensor = torch.full((h * w, 1), strides[0], dtype=x.dtype, device=x.device)
